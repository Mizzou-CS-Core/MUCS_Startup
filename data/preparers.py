...
def prepare_assignment_table(config: Config):
    ...
    gen_table.mkdir()
    prepare_assignment_window_and_config(config_path=gen_table, canvas_token=config.api_token, canvas_course_id=config.course_ids, canvas_assignment_name_predicate=canvas_assignment_name_predicate, canvas_assignment_phrase_blacklist=blacklist, mucs_instance_code=config.class_code, sqlite3_path=config.sqlite_db_path)
